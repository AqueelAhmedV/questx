# Generated by Django 4.0 on 2024-02-28 08:49

import django.core.validators
from django.db import migrations, models
import django.db.models.deletion
import questx.models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('accounts', '0005_loginmodel'),
    ]

    operations = [
        migrations.CreateModel(
            name='Experience',
            fields=[
                ('exp_id', models.CharField(editable=False, max_length=9, primary_key=True, serialize=False, unique=True)),
                ('agent_name', models.CharField(max_length=200)),
                ('agent_location', models.CharField(max_length=512)),
                ('agent_phone', models.CharField(max_length=15, validators=[django.core.validators.RegexValidator(code='invalid_phone_number', message='Phone number must be in the format +81XXXXXXXXXX or 0XXXXXXXXXX', regex='^(\\+?81|0)-?(\\d{1,4})-?(\\d{1,4})-?(\\d{4})$')])),
                ('exp_type', models.CharField(choices=[('task', 'Task'), ('activity', 'Activity/Event')], max_length=10)),
                ('exp_title', models.CharField(max_length=200)),
                ('exp_description', models.CharField(max_length=512, validators=[django.core.validators.MinLengthValidator(50, 'Please describe your task in atleast 50 characters')])),
                ('exp_duration', models.PositiveIntegerField()),
                ('exp_preferred_time', models.CharField(choices=[('night', 'Night'), ('day', 'Day')], default='day', max_length=10)),
                ('quest_id', models.CharField(blank=True, max_length=10, null=True)),
            ],
        ),
        migrations.CreateModel(
            name='ManagerProfile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('joined_on', models.DateTimeField(auto_now_add=True, verbose_name='Join date')),
                ('location', models.CharField(max_length=512)),
                ('organization', models.CharField(max_length=512)),
                ('user', models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='cm_profile', to='accounts.customuser')),
            ],
        ),
        migrations.CreateModel(
            name='Quest',
            fields=[
                ('quest_id', models.CharField(editable=False, max_length=9, primary_key=True, serialize=False, unique=True)),
                ('quest_title', models.CharField(max_length=200)),
                ('quest_description', models.CharField(max_length=512, validators=[django.core.validators.MinLengthValidator(20, 'Description should be minimum 20 characters')])),
                ('quest_duration', models.PositiveIntegerField()),
                ('quest_tags', models.JSONField(blank=True, default=list, null=True)),
                ('exp_ids', models.JSONField(default=list, validators=[questx.models.ExperienceValidator()])),
                ('cm', models.ForeignKey(default=None, on_delete=django.db.models.deletion.CASCADE, related_name='quests', to='questx.managerprofile')),
            ],
        ),
        migrations.CreateModel(
            name='MemberProfile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('phone', models.CharField(max_length=15, validators=[django.core.validators.RegexValidator(code='invalid_phone_number', message='Phone number must be in the format +81XXXXXXXXXX or 0XXXXXXXXXX', regex='^(\\+?81|0)-?(\\d{1,4})-?(\\d{1,4})-?(\\d{4})$')])),
                ('date_of_birth', models.DateField(verbose_name='Date of birth')),
                ('place_of_birth', models.CharField(max_length=512)),
                ('place_of_residence', models.CharField(max_length=512)),
                ('members_details', models.JSONField(default=list)),
                ('field of specialization', models.JSONField(default=list)),
                ('hobby', models.JSONField(default=list)),
                ('notices', models.JSONField(default=list)),
                ('expectations', models.CharField(max_length=512)),
                ('concern', models.CharField(max_length=512)),
                ('user', models.OneToOneField(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='member_profile', to='accounts.customuser')),
            ],
        ),
        migrations.AddConstraint(
            model_name='memberprofile',
            constraint=models.UniqueConstraint(fields=('user',), name='unique_member_profile'),
        ),
        migrations.AddConstraint(
            model_name='managerprofile',
            constraint=models.UniqueConstraint(fields=('user',), name='unique_cm_profile'),
        ),
    ]
